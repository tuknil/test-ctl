...
- Shared attack contracts v2: `docs/shared-attack-contracts-v2.md`
...
- Additively accepts `control-translation@2.0` with exact Check Generation,
  Defense Generation, Mitigation Check, and Bypass Validation locators. It
  verifies the complete shared-contract join before policy, translation, or
  model calls, then translates every cooperating WAF artifact and directive.
...
- **Real:** FastAPI HTTP surface, Pydantic contracts, exact Databricks SQL
  reads from three legacy or four v2 upstream result tables, cross-record lineage and route
...
required obligation. If any required source artifact, carrier, directive, or
obligation cannot map, the result is `cannot-express` and contains no partial
target artifacts or mappings. See `docs/shared-attack-contracts-v2.md`.
